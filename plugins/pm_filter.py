...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    if not pmspoll:
        ...
        if 2 < len(message.text) < 100:
           ...
           # files, offset, total_results = await get_search_results(search.lower(), offset=0, filter=True)
            ...
            await syd.edit("Fɪʟᴇꜱ ꜰᴏᴜɴᴅ! ꜱᴏʀᴛɪɴɢ...")
        else: return 
    else:
  ...
  #  btn.insert(0, [InlineKeyboardButton("🔗 ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ 🔗", "howdl")])
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f" Pᴀɢᴇꜱ 1/{math.ceil(int(total_results) / 6)}", callback_data="pages"),
            InlineKeyboardButton(text="Nᴇxᴛ ➡️", callback_data=f"pmnext_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text=" Pᴀɢᴇꜱ 1/1", callback_data="pages")]
        )
